    ...
    <div className="w-full">
      <div className="flex gap-6 md:gap-10 mb-4 bg-white w-full">
        ...
        <div className="flex flex-col justify-center">
          ...
          <p className="capittalize text-gray-400 md:text-xl text-xs">{user.userName}</p>
        </div>
      </div>
      <div>
        ...
        <div className="flex gap-6 flex-wrap md:justify-start">
          {videosList.length > 0 ? (
            videosList.map((video: Video, idx: number) => <VideoCard key={idx} post={video} />)
          ) : (
            <NoResults text={`No ${showUserVideos ? "" : "Liked"} Videos Yet`} onComments={false} />
          )}
        </div>
      </div>
    </div>
